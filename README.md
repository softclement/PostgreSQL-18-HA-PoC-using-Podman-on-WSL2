...
Confirm `host=pg-primary` appears in the `primary_conninfo` line. Because you passed `-h pg-primary` to `pg_basebackup`, this should already be correct.

If the host value is wrong for any reason, fix it:

```bash
sed -i "s/host='[^']*'/host='pg-primary'/" \
  ~/pg-podman-ha/standby_base/postgresql.auto.conf
```
